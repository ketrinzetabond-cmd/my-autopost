...
# 4. Вечный двигатель (С твоими правками)
async def check_and_send():
    now = datetime.now() + timedelta(hours=2) 
    now_key = now.strftime("%Y-%m-%d %H:%M")
    
    # ПРАВКА №1: SQL склейка через ||
    c.execute("""
    ...
    for p_id, txt in pending:
        try:
            # ПРАВКА №3: Убрали parse_mode для стабильности
...
# ПРАВКА №2: Настройка очереди планировщика
if 'scheduler_started' not in st.session_state:
    ...
    scheduler.add_job(
        ...
        'interval', 
        minutes=1, 
        max_instances=1,  # Не запускать копию, если прошлая еще работает
        coalesce=True     # Склеивать пропущенные запуски в один
    )
    scheduler.start()
